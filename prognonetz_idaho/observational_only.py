# implementation of torch dataset using only observational data and no preprocessing
...
class PrognonetzDataSet(data.Dataset):
    ...
    # metadata
    url = "http://www.dropbox.com/s/qwjtwfzl4ei2i05/prognonetz_dataset.tar.gz?dl=1"
    ...
    def __init__(self, root, split='train', download=False, small=True,
                 input_shape=None, output_shape=None, zerofilling=True):

        # change metadata to small set
        if small:
            self.url = "http://www.dropbox.com/s/gk72osjlcjg14fh/prognonetz_dataset_small.tar.gz?dl=1"
        ...
        if input_shape is None:
            self.input_shape = ['RWM_temperature_15m', 'RWM_wind_speed', 'RWM_wind_direction',
