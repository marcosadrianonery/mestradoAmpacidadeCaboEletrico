...
def split_with_seed(length, split, seed, val_size, splittype='random', chunklength=200):
    ...
    if splittype is 'random':
        sample = np.random.mtrand.RandomState(seed).choice(
            range(length), int(val_size*length))
        if split is 'val':
            return sample
        else:
...
class PrognonetzDataSet(data.Dataset):
    ...
    def __init__(self, root, split='train', download=False, small=True,
                 nwp=True, obs=True, input_shape_obs=None, input_shape_nwp=None,
                 output_shape=None, output_transform=None, input_transform_obs=None,
                 input_transform_nwp=None, split_seed=0, val_size=0.2,
                 zerofilling=True, mask=False, age_matrix=False, nearestneighbour=None,
                 splittype='chunks'):
        ...
        if small:
            ...
            self.train_list = [
                ['Prognonetz_INL_train_small.npz',
                    '1dab55823d26488165d6eddcc1d25cf0'],
            ]
            self.test_list = [
                ['Prognonetz_INL_test_small.npz',
                    '44456c97682c3211a3ee2038368328ed'],
            ]
        ...
        if output_shape is None:
            self.output_shape = ['RWM_690', '690_RWM',
        ...
        if nwp:
            ft_nwp_index = list(np.load(os.path.join(
                self.root, filename))['ft_nwp_index'])
            ...
            if nearestneighbour is not None:
                self.features_nwp = np.load(os.path.join(self.root, filename))[
                    'ft_nwp'][..., nearestneighbour[0], nearestneighbour[1], ft_nwp_shape]
            else:
                self.features_nwp = np.load(os.path.join(self.root, filename))[
                    'ft_nwp'][..., ft_nwp_shape]
            for vector in vectors:
                # vectorize selected values
                self.features_nwp[..., vector] = polar_to_cartesian(
                    self.features_nwp[..., vector])
            # perform processing
            self.features_nwp = self.input_transform_nwp.process(
                self.features_nwp)
            ...
            if mask:
                ...
                # append mask to data
                self.features_nwp = np.concatenate(
                    (self.features_nwp, mask_nwp), axis=-1)
            if age_matrix:
                # load weights for weight matrix
                self.nwp_weights = np.load(os.path.join(
                    self.root, filename))['nwp_weights']
                # create mask of ones if not nan and zeros else
                age_matrix_nwp = np.ones(pre_mask_shape)
                if nearestneighbour is not None:
                    age_matrix_nwp = age_matrix_nwp * \
                        self.nwp_weights[:, :, None]
                else:
                    age_matrix_nwp = age_matrix_nwp * \
                        self.nwp_weights[:, :, None, None, None]
                age_matrix_nwp[pre_mask_nans] = float(0)
                # append mask to data
                self.features_nwp = np.concatenate(
                    (self.features_nwp, age_matrix_nwp), axis=-1)
            if zerofilling:
                # fill nan with zero
                self.features_nwp[np.isnan(self.features_nwp)] = float(0)
        if obs:
            ft_obs_index = list(np.load(os.path.join(
                self.root, filename))['ft_obs_index'])
            shape, vectors = parse_shape(self.input_shape_obs)
            ft_obs_shape = [ft_obs_index.index(item) for item in shape]
            self.features_obs = np.load(os.path.join(self.root, filename))[
                'ft_obs'][..., ft_obs_shape]
            for vector in vectors:
                self.features_obs[..., vector] = polar_to_cartesian(
                    self.features_obs[..., vector])
            self.features_obs = self.input_transform_obs.process(
                self.features_obs)
            if zerofilling:
        ...
        self.labels = self.output_transform.process(
            np.load(os.path.join(self.root, filename))['lb'][:, :, lb_shape])
        if selected_samples is not None:
            selected_samples = split_with_seed(
                len(self.labels), self.split, split_seed, val_size, splittype=splittype)
